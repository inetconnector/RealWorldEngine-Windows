...
### Gradle/Android build error: “file is located outside the root directory”

If you see an error like:

> The given file `...\EmbeddingProvider$encode$2$2.class` is located outside the root directory `...`

this usually means Gradle/AGP is reading stale build artifacts from a previous checkout path (for example, after moving the repo, changing drive letters, or using a `SUBST`/symlink). To fix it:

1. **Delete build outputs** in the affected module(s): `build/`, `intermediates/`, and any `.gradle/` folders inside the project.
2. **Clear Gradle caches** (optional but often needed on Windows after path changes):
   - `%USERPROFILE%\.gradle\caches`
   - `%USERPROFILE%\.gradle\build-cache`
3. **Rebuild from a stable, non-symlinked path** (avoid moving the repo between drives after a build).
4. If the problem persists, **disable the Gradle build cache** temporarily (`org.gradle.caching=false`) to ensure no stale transforms are reused.

These steps force Gradle/AGP to regenerate dexing inputs using the current project path.
